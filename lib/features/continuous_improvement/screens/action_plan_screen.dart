...
class _ActionPlanScreenState extends State<ActionPlanScreen> {
  ...
  Future<void> _fetchPlans() async {
    ...
    try {
      final newPlans = await dataService.getActionPlans(filter.shift, filter.fleet, filter.period);
      if (mounted) {
        setState(() {
          _plans = newPlans;
          _isLoading = false;
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Erro ao carregar dados: $e"), backgroundColor: AppTheme.alertaCritico),
        );
      }
    }
  }

  void _showAddPlanDialog(BuildContext context, FilterProvider filter, DataService dataService) {
              ...
              onPressed: () async {
                if (titleController.text.isNotEmpty && responsibleController.text.isNotEmpty) {
                  try {
                    ...
                    if (mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("Erro ao salvar: $e"), backgroundColor: AppTheme.alertaCritico),
                      );
                    }
                  }
                }
  ...
  Widget build(BuildContext context) {
    ...
    if (filter.errorMessage != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(filter.errorMessage!), backgroundColor: AppTheme.alertaCritico),
        );
        filter.clearError();
      });
    }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_isLoading || filter.isLoading)
             const Expanded(child: Center(child: CircularProgressIndicator(color: AppTheme.verdeVale)))
